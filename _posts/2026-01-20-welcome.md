---
layout: post
title: "Vibe Coding a Mobile App with AI Coding Agents to 'Shop Smart' "
date: 2026-01-20 10:00:00 +0530
tags: [mobile-app, android, ai-agents, llm, azure]
---
...
two bank accounts and Sampath Bank and Commercial bank having two credit cards with them. Then your SMS inbox will look like below. <br> It is likely that you'd miss out an actually good offer due to information overload. <br>
...
[QA and error analysis](#erroranalysis)<br>
[Setting up CICD for backend](#cicdbackend)<br>
[Solution Diagram - Jan 2026](#solutiondiagramjan2026)<br>
[Whats Next ](#nextfeatures)<br>
...
When I decided to have a seperate service for backend, the backend project had a seperate spec. Specially specifying how it should interact with the Mobile app. In this spec I've included the details about JSON payloads it must process, how internal states should be maintained (_in first iteration it was file based. But I made provision to integrate to a document database_). By this time I've decided to host the backend in Azure (_for learning purposes of course_). <br>
I chose Azure Container Applications ( ACA) & Cosmos DB ( Document database) as my Azure Services. <br> Both of them provide generous free tier. Upon consulting AI, this is the same reason I ditched Azure Contaier Registry ( ACR ) for GCR. <br>
...
The repo can be found here; [repo](https://github.com/nilanka-weeraman/promoapp-backend)

Amazon has [KIRO](https://kiro.dev/) which explictly says it is a spec driven AI coding tool. I've experienced that Amazon's Q getting more intelligent (  _powered by Claude 4 & above_). There is every reason to try KIRO out., but for this endevour I chose Antigravity <br>


## QA and error analysis {#erroranalysis}

[Andrew NG recently highlighted the importance of evals & error analysis as a differentiator](https://learn.deeplearning.ai/courses/agentic-ai/lesson/0kbds1/more-error-analysis-examples) and [something machine learning practioners can carry to Agentic AI development](https://www.linkedin.com/posts/andrewyng_deepseek-cuts-inference-costs-openai-tightens-activity-7384633283554447360-sVXe). <br>
...
However I found it hard to use AI CLI s with notebooks, since I've experienced Colab with Gemini providing a greater integration. When doing data anlysis on notebooks with Codex, CC, Antigravity the notebook gets re-created wiping out the results of cell exeuction. When you're working on VSCode and have another agent like Q, you can generate code within the notebook, which is much more helpful and intuitive experience than using CLI tools with notebooks. Nevertheless an exciting area to explore and I haven't seen much material on this.
...
In my Azure Container Apps ( ACA ) instance, I configured GCR as a remote repository. <br>

All the Azure configurations were assisted by Antigravity. On my request it created a checklist and I've implemented each step. It was like following a lab tutorial. I was quite surprised the accuracy / upto date nature of the deployment guide that Antigravity producedw with Gemini 3.0 pro.  <br>
...
I've 
_Note : I decided to use Azure solely to tryout another cloud apart from AWS. And it wasn't a case of choosing better services after evaluation_
<br>

## Solution Diagram - Jan 2026 {#solutiondiagramjan2026}

Below represents a simple archi diagram of how different components are deployed in my workflow. <br>
There are no authentication mechanisms implemented yet ! <br>

<img class="post-image" style="--post-image-width: 350px; --post-image-max: 350px;" src="{{ '/assets/images/archi-jan.drawio.png' | relative_url }}" alt="Archi Diagram" width="350" height="350" />

## Whats Next {#nextfeatures}

I've build an app version that collects events from the mobile app. I've used Azure Events SDK. <br>
I was able to succesfully emit events and collect them in ADLS. <br>Next I'll be setting up data pipelines using Azure Data Factory or Fabric , store processed data in Synapse / One lake.<br>
Final plan is to connect a powerBI dashboard to see near-realtime insights ( very naive insights :-) ) 
